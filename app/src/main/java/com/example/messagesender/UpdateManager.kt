...
/**
 * Self-hosted in-app updater. Checks <server>/app/version.json for a newer
 * versionCode and, if found, offers to download and install
 * <server>/app/alfa-sms.apk. Android still shows its own install confirmation.
 */
object UpdateManager {
    ...
    private fun base() = BuildConfig.SERVER_URL.trimEnd('/')

    fun checkForUpdate(activity: Activity) {
        if (BuildConfig.SERVER_URL.contains("your-server.example.com")) return
        io.execute {
            try {
                val json = httpGet("${base()}/app/version.json") ?: return@execute
                ...
                if (latest > BuildConfig.VERSION_CODE) {
                    main.post {
                        if (!activity.isFinishing) promptUpdate(activity, name, notes)
                    }
                }
            } catch (e: Exception) {
    ...
    private fun startUpdate(activity: Activity) {
        // Android O+ requires per-app permission to install packages.
    ...
    private fun downloadApk(activity: Activity): File? {
        return try {
            ...
            val conn = URL("${base()}/app/alfa-sms.apk").openConnection() as HttpURLConnection
